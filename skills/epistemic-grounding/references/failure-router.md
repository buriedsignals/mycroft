...
This reference adapts the WFGY problem-map idea into Mycroft's claim-verification workflow. Mirrored from the Spotlight bundle's copy — both skills share the same router because the failure classes are tool-agnostic.
...
For Mycroft and Spotlight, the critical family is **Grounding & Evidence Integrity**: the claim has lost reliable tie to anchors, referents, or evidence.
...
Escalate to Spotlight casework or a follow-up investigation when:
...
- the claim would be ingested into the knowledge vault under `confidence: high`.
