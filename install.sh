...
- Goose schedules: mycroft-morning-brief at 07:00, mycroft-wiki-audit at 18:15
...
Use OpenKnowledge for durable local search before broad web search when the answer may already be in the Mycroft wiki.
Do not use Mycroft as the active OSINT case workspace.

## Getting Started Route

When the user is new after install, asks how to start, or the wiki contains only scaffold/example files, do not stop at "nothing found." Explain that Mycroft needs reporting context or source material, then offer:
...
- _audits/ stores scheduled wiki audit reports.
...
type: start-here
title: Start Here
description: First actions for a new Mycroft wiki.
...
If the wiki is still empty, start by adding material or defining your beat. Wiki audits are useful later, after you have real notes.
VAULT_START_EOF
...
instructions: |
  ...
  Cite every item and write the brief into the Mycroft wiki.
...
SCHEDULED_MORNING_EOF
  ...
  cat > "$MYCROFT_GENERATED_RECIPES/wiki-audit.scheduled.yaml" <<SCHEDULED_AUDIT_EOF
version: "1.0.0"
title: "Mycroft scheduled wiki audit"
description: "Generated by Mycroft setup. Audits the configured Mycroft OpenKnowledge wiki."

instructions: |
  Load the Mycroft journalism instructions and knowledge-primitives skill.
  Audit the configured Mycroft wiki. Do not rewrite user notes; write a dated report.
...
prompt: |
  Run the Mycroft wiki audit using these configured paths:

  - Mycroft wiki: $VAULT_PATH
  - Report directory: $VAULT_PATH/_audits

  Follow $MYCROFT_DIR/recipes/wiki-audit.yaml exactly. Write the result to:

  $VAULT_PATH/_audits/wiki-audit-YYYY-MM-DD.md
SCHEDULED_AUDIT_EOF
...
install_goose_schedules() {
  ...
  goose schedule remove --schedule-id mycroft-wiki-audit >/dev/null 2>&1 || true
  ...
  if goose schedule add --schedule-id mycroft-wiki-audit --cron "0 15 18 * * *" --recipe-source "$MYCROFT_GENERATED_RECIPES/wiki-audit.scheduled.yaml" >/dev/null 2>&1; then
    ok "Goose schedule mycroft-wiki-audit"
  else
    warn "Could not create Goose wiki-audit schedule; use Goose Scheduler with $MYCROFT_GENERATED_RECIPES/wiki-audit.scheduled.yaml"
  fi
}
...
mkdir -p "$VAULT_PATH" && ok "Mycroft wiki $VAULT_PATH"
ensure_goose
