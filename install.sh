#!/usr/bin/env bash
# Mycroft is installed through Engine. Indicator Labs provides the managed path.
# This file remains so old curl|bash pipes fail closed instead of fetching a
# public-installer bundle or opening a localhost configure page.
set -euo pipefail

JOIN='https://buriedsignals.com/join'

cat <<EOF
This script is a fail-closed pointer; it does not install Mycroft.
