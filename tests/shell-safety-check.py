#!/usr/bin/env python3
"""Hostile-input regression test for `mycroft_safe.py`.

Mirrored from Spotlight's `shell-safety-check.py`. Run after any change to
`mycroft_safe.py` to confirm all known attack patterns still reject.
"""
import subprocess
import sys
import tempfile
...
HELPER = ROOT / "scripts" / "mycroft_safe.py"
