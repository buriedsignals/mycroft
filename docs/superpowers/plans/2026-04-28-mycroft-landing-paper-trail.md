...
# PHASE D — Final review (Tasks 20–22)

## Task 20: Performance + console pass

**Files:**
- Modify: `prototypes/proto-13/index.html`

- [ ] **Step 1:** Pause the RAF when no pose is animating significantly and the user is idle. Track per-tick max delta:

```js
let maxDelta = 0;
// at end of for-loop, accumulate:
//   maxDelta = Math.max(maxDelta, Math.abs(sectionTargets[ix]-px));
// outside the for-loop:
if (activePose !== 'hero' && maxDelta < 0.001) {
  // settled — request only every 250ms instead of every frame
  setTimeout(() => { raf = requestAnimationFrame(tick); }, 250);
  return;
}
maxDelta = 0;
raf = requestAnimationFrame(tick);
...
- [ ] **Step 2:** Open DevTools → Performance, record 10s of scroll. Verify ≥45fps. If under: lower COUNT to 600 unconditionally, or simplify plugin modulation.

- [ ] **Step 3:** Verify console is clean (no errors, no warnings beyond expected font-load).

- [ ] **Step 4:** Commit.

```bash
git add prototypes/proto-13/index.html
git commit -m "perf(proto-13): idle-pause RAF when poses settle"
```

---

## Task 21: Full browser-use review
...
```bash
bunx browser-use --url http://localhost:5180/ --task "Full landing review. (1) Take screenshot of hero. (2) Move mouse across hero, observe paper repulsion. (3) Slowly scroll to footer. At each section (day, plugins, pricing, privacy, trusted, studio, cta, footer) wait 2s and screenshot. (4) Click each pricing tab. (5) Click each day-list item 01-04, screenshot the right paper updating. (6) Click 'Set up Mycroft' button — observe paper burst. (7) Report any console errors, layout issues, jank, or moments where particles disappear unexpectedly. Be thorough."
```

- [ ] **Step 2:** Address any issues reported. Iterate until clean.

- [ ] **Step 3:** Commit.

---

## Task 22: Final smoke test + close-out

**Files:**
- Modify: `prototypes/proto-13/index.html`

- [ ] **Step 1:** Test no-JS: in DevTools, disable JavaScript, reload. Verify all sections readable, layout intact (only the canvas and interactivity are missing). The static `.hero .brand-mark` text serves as a fallback for the hero brand.

- [ ] **Step 2:** Test reduced-motion: System Settings → Accessibility → Display → Reduce motion ON, reload, verify scene snaps to poses without animation, hero is static.

- [ ] **Step 3:** Verify the existing `index.html` and all other prototypes are untouched.

```bash
git status
# Should show only: prototypes/proto-13/ as modified/created
```

- [ ] **Step 4:** Final commit (if any tweaks).

```bash
git add -A prototypes/proto-13/
git commit -m "feat(proto-13): close out paper-trail landing prototype"
```
