...
<div class="shell">
  ...
  <table>
    ...
    <tr><th>Knowledge project</th><td><code>$vault</code> — your durable newsroom memory in portable Markdown</td></tr>
...
$plugins_row$optional_row  </table>
  ...
  <p class="num">$n_first — First run</p>
...
def build_getting_started(d):
    ...
    return GETTING_STARTED_TEMPLATE.substitute(
        ...
        vault=esc(d["vault"]),
        spotlight_vault_row=(
            '    <tr><th>Spotlight</th><td>Install separately in Indicator Labs; Mycroft records handoff if Spotlight is already present</td></tr>\n'
